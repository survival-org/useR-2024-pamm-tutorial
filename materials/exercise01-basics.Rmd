...
Visualize the estimated (log-)baseline hazard (using the `mgcv::plot` function)
...
```{r}
ndf_tumor <- tumor_ped %>%
  make_newdata(tend = unique(tend), strata = unique(strata), age = c(0), sex = c("male"), transfusion = c("no"), resection = c("no")) %>%
...
8. Now use `add_surv_prob` to add to the data set the estimated survival probabilities and visualize those.
...
ggplot(ndf_tumor, aes(x = tend, y = surv_prob)) +
