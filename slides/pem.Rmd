...
Cox model $$h(t|\bfx_i) = h_0(t) \exp(\bfx_i^\top\bsbeta) = \exp(g_0(t) + \bfx_i^\top \bsbeta)$$
where the baseline hazard is written as $g_0(t) = \log h_0(t)$. <br>
We now want to estimate both $h_0(t)$ as well as the parameters
$\bsbeta$ via ML.

We again decompose the time axis into $J$ intervals, using cut points $a_0, a_1, \ldots, a_J$:
...
$$\begin{align*}
L(\bsbeta,\bsbeta_0)
  &= \prod_{i=1}^n h(t_i|\bfx_i)^{\delta_i} \exp
    \left(
      -\int_0^{t_i} h(s|\bfx_i) ds
    \right),
\end{align*}$$
where $\bsbeta_0 = (\beta_{01},\ldots,\beta_{0J})$. <br>
Since $h(s|\bfx_i)$ is piecewise constant, the integral in the log-likelihood can be easily solved.  <br>
...
$$\text{Since }\quad h(t_i|\bfx_i)^{\delta_i}
  = \prod^J_{j=1} \exp \left( \delta_{ij} \eta_{ij}\right)
  = \prod^{J_i}_{j=1} \exp \left( \delta_{ij} \eta_{ij}\right),$$
where $J_i$ the interval for which $t_i \in I_{J_i}=(a_{J_i-1},a_{J_i}]$, and
$$\int_0^{t_i} h(s|\bfx_i) ds
...
Now assume $\delta_{ij} \stackrel{iid}{\sim} Po(\mu_{ij})$, with $\mu_{ij} = h_{ij}t_{ij}$ and density $f(\delta_{ij}) = \mu_{ij}^{\delta_{ij}}/\delta_{ij}!\cdot \exp(-\mu_{ij})$
...
In general
  ...
  - If no ties in the data, $\hat{\bsbeta}_{PEM}=\hat{\bsbeta}_{Cox}$ `r Citep(bib, "whitehead_fitting_1980")`
...
**Solution**:
- large $J$, penalize differences between neighboring baseline hazards $\beta_{0j}, \beta_{0j'}$ and/or

- reduction of parameter dimension by *basis function representation* of $h_0(t)$.
...
Impose a penalty on the step heights $$pen(\bsbeta_0) = \sum_{j=2}^J (\beta_{0j} - \beta_{0j-1})^2,$$
...
with
$$l_{pen}(\bsbeta,\bsbeta_0) = l(\bsbeta,\bsbeta_0) - \tau \ pen(\bsbeta_0)$$ and
...
- $\tau \longrightarrow \infty$ we obtain $g_0(t)=const$

$\ra$ It is therefore necessary to choose $\tau$ in a data-driven way.

$\ra$ As we simply optimise a (penalized) Poisson log-likelihood, we can use any optimization strategy for GLMs
...
- A good strategy is to select cut points at observed event times $t_{(1)},\ldots,t_{(m)}$ (basis functions will be evaluated in regions with many events, i.e., regions with a lot of information about the process)<br>
- For large $m$, a small subsample will often be sufficient <br>
- Number of parameters $L$ does not depend on number of cut points $J$ (but computation time still depends on $J$ to some degree) <br>
- PEMs and PAMMs in **R** facilitated by package **`pammtools`** <br>
- Various articles (vignettes) on different topics available here: https://adibender.github.io/pammtools/articles/
